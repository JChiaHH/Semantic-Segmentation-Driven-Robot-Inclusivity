...
class S3DIS(BaseDataset):
    ...
    def create_ply_files(dataset_path, class_names):
        ...
        # 1) Discover annotation folders dynamically (works with room_000xx layout)
        anno_paths = sorted(Path(dataset_path).rglob("Annotations"))
        anno_paths = [p for p in anno_paths if p.is_dir()]

        class_names = [val for _, val in class_names.items()]
        label_to_idx = {l: i for i, l in enumerate(class_names)}

        out_format = ".pkl"

        for anno_path in tqdm(anno_paths):
            # save name: Area_1_room_00000.pkl
            # anno_path: .../Area_1/room_00000/Annotations
            room_dir = anno_path.parent
            area_dir = room_dir.parent
            save_name = f"{area_dir.name}_{room_dir.name}{out_format}"
            save_path = Path(dataset_path) / "original_pkl" / save_name

            txts = list(anno_path.glob("*.txt"))
            if len(txts) == 0:
                continue
            ...
            for file in txts:
                stem = Path(file).stem                      # "Containers_And_Pallets_-1"
                class_name = stem.rsplit("_", 1)[0]         # "Containers_And_Pallets"

                if class_name not in label_to_idx:
                    class_name = "Unlabelled"               # safer fallback for your label set

                # pc = pd.read_csv(file, header=None, delim_whitespace=True).values
                # if pc.size == 0:
                #     continue

                # labels = np.full((pc.shape[0], 1), label_to_idx[class_name], dtype=np.float32)
                # data_list.append(np.concatenate([pc, labels], axis=1))

                # bbox = [class_name] + get_min_bbox(pc)
                # bboxes.append(bbox)

                pc = pd.read_csv(file, sep=r"\s+", header=None).values

                # ---- guards ----
                if pc is None or pc.size == 0:
                    continue

                # sometimes pandas returns shape (N,) if a line is malformed
                if pc.ndim != 2:
                    continue

                # need at least XYZ, and ideally RGB too
                # many S3DIS annotations are x y z r g b (6 cols)
                if pc.shape[1] < 3:
                    continue

                # if you rely on RGB later (feat = pc[:,3:6]) ensure >=6
                if pc.shape[1] < 6:
                    # either pad RGB with zeros...
                    pad = np.zeros((pc.shape[0], 6 - pc.shape[1]), dtype=pc.dtype)
                    pc = np.hstack([pc, pad])

                # bbox computation needs enough points
                if pc.shape[0] < 3:
                    continue
                ...
                # now safe:
                labels = np.full((pc.shape[0], 1), label_to_idx[class_name], dtype=np.float32)
                data_list.append(np.concatenate([pc[:, :6], labels], axis=1))
                bbox = [class_name] + get_min_bbox(pc[:, :3])
                bboxes.append(bbox)

            if len(data_list) == 0:
                continue

            pc_label = np.concatenate(data_list, axis=0)
            with open(save_path, "wb") as f:
                pickle.dump([pc_label, bboxes], f)
