...
class SemanticSegmentation(BasePipeline):
    ...
    **Returns:**
    ...
    def run_test(self):
        ...
        self._test_files_completed = 0  # SEMSEG_PATCH
        self._test_files_total = len(test_dataset)  # SEMSEG_PATCH
        log.info(f"Total test files: {self._test_files_total}")  # SEMSEG_PATCH
        ...
        with torch.no_grad():
            for unused_step, inputs in enumerate(test_loader):
                ...
                if self.complete_infer:
                    ...
                    self._test_files_completed += 1  # SEMSEG_PATCH
                    log.info(f"[{self._test_files_completed}/{self._test_files_total}] Completed: {attr['name']} - saved predictions")  # SEMSEG_PATCH
        ...
        # SEMSEG_PATCH: Guard against empty metrics (no ground truth labels)
        try:
            acc_list = self.metric_test.acc()
            iou_list = self.metric_test.iou()
            if len(acc_list) > 0 and len(iou_list) > 0:
                log.info(f"Overall Testing Accuracy : {acc_list[-1]}, mIoU : {iou_list[-1]}")
            else:
                log.info("No ground truth labels available - metrics not computed")
        except Exception as e:
            log.info(f"Metrics not available (no ground truth): {e}")
    ...
    def run_train(self):
        ...
        train_loader = DataLoader(
            ...
            worker_init_fn=lambda worker_id: np.random.seed((int(torch.utils.data.get_worker_info().seed) + worker_id) & 0xFFFFFFFF)
        ...
        valid_loader = DataLoader(
            ...
            worker_init_fn=lambda worker_id: np.random.seed(
                (int(torch.utils.data.get_worker_info().seed) + worker_id) & 0xFFFFFFFF
                ),
        )
        ...
        is_resume = model.cfg.get('is_resume', True)
        loaded_epoch = self.load_ckpt(model.cfg.ckpt_path, is_resume=is_resume)

        # If the checkpoint was saved at epoch N (after finishing epoch N),
        # the next epoch to run should be N+1.
        start_epoch = (loaded_epoch + 1) if is_resume and loaded_epoch is not None else 0
        start_epoch = max(0, start_epoch)

        if start_epoch > cfg.max_epoch:
            log.warning(
                f"Checkpoint epoch {loaded_epoch} >= max_epoch {cfg.max_epoch}. Nothing to train."
            )
            return
        ...
        dataset_name = dataset.name if dataset is not None else ''
        base_tb_name = model.__class__.__name__ + '_' + dataset_name + '_torch'
        base_tb_path = join(self.cfg.train_sum_dir, base_tb_name)

        if is_resume:
            # Try to reuse the latest existing run directory
            if exists(self.cfg.train_sum_dir):

                existing_runs = [
                    d for d in Path(self.cfg.train_sum_dir).glob(f"*_{base_tb_name}")
                    if d.is_dir()
                ]


                if len(existing_runs) > 0:
                    # Pick the most recently updated run folder
                    existing_runs = sorted(existing_runs, key=lambda p: p.stat().st_mtime)
                    self.tensorboard_dir = str(existing_runs[-1])
                    log.info(f"Resuming TensorBoard logdir: {self.tensorboard_dir}")

                else:
                    # No previous runs found → create new
                    runid = get_runid(base_tb_path)
                    self.tensorboard_dir = join(self.cfg.train_sum_dir, runid + '_' + base_tb_name)

            else:
                runid = get_runid(base_tb_path)
                self.tensorboard_dir = join(
                    self.cfg.train_sum_dir,
                    runid + '_' + base_tb_name
                )
        ...
        for epoch in range(start_epoch, cfg.max_epoch + 1):
            ...
            # epochs_done = epoch + 1
            # epochs_total = cfg.max_epoch + 1
            # avg_epoch_time = elapsed / epochs_done
            # estimated_total = avg_epoch_time * epochs_total
            ...
            epochs_total = (cfg.max_epoch - start_epoch) + 1
            ...
            # Log every epoch, or you can do "if epoch % 5 == 0" to reduce spam
            log.info(
                f"[TIME] Epoch {epoch} took {epoch_time/60:.2f} min | "
    ...
    def save_logs(self, writer, epoch):
        ...
        # ✅ NEW: per-class curves (skip last element = mean)
        num_classes = len(train_accs) - 1
        for c in range(num_classes):
            writer.add_scalar(f"Accuracy/Train_class_{c}", train_accs[c], epoch)
            writer.add_scalar(f"Accuracy/Val_class_{c}",   val_accs[c],   epoch)
            writer.add_scalar(f"IoU/Train_class_{c}",      train_ious[c], epoch)
            writer.add_scalar(f"IoU/Val_class_{c}",        val_ious[c],   epoch)
    ...
    def _infer_epoch_from_ckpt_path(self, ckpt_path: str) -> int:
        """Fallback: infer epoch from ckpt filename like ckpt_00020.pth."""
        name = Path(ckpt_path).name
        m = re.search(r'ckpt_(\d+)\.pth$', name)
    ...
    def load_ckpt(self, ckpt_path=None, is_resume=True):
        """Load a checkpoint. If resuming, returns the epoch stored in the ckpt."""
        train_ckpt_dir = join(self.cfg.logs_dir, 'checkpoint')
        make_dir(train_ckpt_dir)
        ...
        #Commented OUT to reset scheduler for fine tuning
        # if 'scheduler_state_dict' in ckpt and hasattr(self, 'scheduler'):
        #     log.info('Loading checkpoint scheduler_state_dict')
        #     self.scheduler.load_state_dict(ckpt['scheduler_state_dict'])

        if is_resume:
            # Prefer epoch stored inside checkpoint; fallback to filename parsing
            loaded_epoch = ckpt.get('epoch', None)
            if loaded_epoch is None:
                loaded_epoch = self._infer_epoch_from_ckpt_path(ckpt_path)
            self.start_epoch = int(loaded_epoch)
            log.info(f"Resuming from checkpoint epoch={self.start_epoch}")
